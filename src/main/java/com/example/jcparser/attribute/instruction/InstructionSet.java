...
//	IMPDEP2("impdep2", 0xFF),                   // 			reserved for implementation-dependent operations within debuggers; should not appear in any class file
	...
	public static Type getOperandsType(int code) {
		...
		type = switch (getInstruction(code)) {
			...
			case BIPUSH -> Type.BYTE;
			case NEWARRAY -> Type.ATYPE;
			case GOTO, GOTO_W, IF_ACMPEQ, IF_ACMPNE, IF_ICMPEQ, IF_ICMPGE, IF_ICMPGT, IF_ICMPLE, IF_ICMPLT, IF_ICMPNE,
			     IFEQ, IFGE, IFGT, IFLE, IFLT, IFNE, IFNONNULL, IFNULL, JSR, JSR_W -> Type.BRANCH_OFFSET;
			case LOOKUPSWITCH -> Type.LOOKUPSWITCH;
			case TABLESWITCH -> Type.TABLESWITCH;
			case WIDE -> Type.WIDE;
			default -> Type.NOP;
		};
		return type;
